...
```
POST /api/v1/declarations/refresh-statuses
```
...
```json
{
  ...
  "code": "AGENT_REQUIRED",
  "hint": "Use the agent flow: POST /api/v1/declarations/refresh-prepare, then relay ANAF's answer to POST /api/v1/declarations/refresh-agent-result. See https://docs.storno.ro/agent"
}
```
...
1. `POST /api/v1/declarations/refresh-prepare` — returns the ANAF URL (and token) the agent must call with the certificate
2. the agent performs the ANAF request (`https://agent.storno.ro:17394/proxy`)
3. `POST /api/v1/declarations/refresh-agent-result` — relay ANAF's raw answer; Storno updates the declarations
